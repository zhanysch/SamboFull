...
abstract class BaseDataSource <T>(  /// Т классов из датаклассов преиспользовать????
    ...
    override fun loadInitial(  // загружает данные
        ...
        scope.launch(Dispatchers.IO){
            kotlin.runCatching {
                val result = getListByPageNumber( limit = limit,page = 0 )  // на превой загрузке загрузка = 0 & limit скачивает 20 итемов
    ...
    override fun loadAfter(params: LoadParams<Int>, callback: LoadCallback<Int, T>) { // загружает дальше данные, после того как лимит прошли
        scope.launch(Dispatchers.IO){
            kotlin.runCatching {
                val result = getListByPageNumber(page = 20 * params.key, limit = limit)  // скачиваем след список
